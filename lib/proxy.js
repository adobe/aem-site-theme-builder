  ...
  proxy.on('proxyRes', function (proxyRes, req, res) {
      ...
      proxyRes.on('end', function () {
        ...
        if (isHtmlModified) {
          try {
            ...
            console.log(`[aem-site-theme-builder] Proxy ${requestUrl} with modified theme artifacts.`);
          } catch (err) {
            console.error('[aem-site-theme-builder] Something went wrong. Proxy html rewrite error: ', err);
          }
        } else {
          res.end(data.toString('binary'));
          console.log(`[aem-site-theme-builder] Proxy ${requestUrl} without changes.`);
        }
      });
    } else {
      proxyRes.pipe(res);
    ...
    for (const header in proxyHeaders) {
      ...
      if (Array.isArray(proxyHeaders[header])) {
        res.setHeader(header, proxyHeaders[header].map(function (val) {
          return val.replace(CONFIG.url, `http://localhost:${CONFIG.port}`);
        }));
      } else {
        res.setHeader(header, proxyHeaders[header].replace(CONFIG.url, `http://localhost:${CONFIG.port}`));
      }
    }
  });
